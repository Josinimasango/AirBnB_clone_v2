...
class TestDBStorageDocs(unittest.TestCase):
    ...
    def test_pep8_conformance_db_storage(self):
        """checks if models/engine/db_storage.py complies with PEP8."""
        pep8s = pycodestyle.StyleGuide(quiet=True)
        result = pep8s.check_files(['models/engine/db_storage.py'])
        self.assertEqual(result.total_errors, 0,)

    def test_pep8_conformance_test_db_storage(self):
        """checks if tests/test_models/test_db_storage.py complies with PEP8."""
        pep8s = pycodestyle.StyleGuide(quiet=True)
...
#         """tests that returns all rows when no class is passed"""
...
#     @unittest.skipIf(storage_t != 'db', "not testing db storage")
#     def test_save(self):
#         """Tests if save does saves objects to file.json"""
...
class TestDBStorageDocs(unittest.TestCase):
    ...
    def test_pep8_conformance_db_storage(self):
        """Tests that models/engine/db_storage.py complies with PEP8."""
        pep8s = pycodestyle.StyleGuide(quiet=True)
        result = pep8s.check_files(['models/engine/db_storage.py'])
        self.assertEqual(result.total_errors, 0,)

    def test_pep8_conformance_test_db_storage(self):
        """checks tests/test_models/test_db_storage.py complies with PEP8."""
        pep8s = pycodestyle.StyleGuide(quiet=True)
        result = pep8s.check_files(['tests/test_models/test_engine/\
test_db_storage.py'])
        self.assertEqual(result.total_errors, 0,)
